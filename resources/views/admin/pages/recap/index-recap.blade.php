...
<div class="content-wrapper">
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Rekapitulasi Belanja Bagi Hasil</h1>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="container-fluid">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Data Rekapitulasi Belanja Bagi Hasil</h3>
                    <div class="card-tools">
                    <ul class="nav nav-pills ml-auto">
                        <li class="nav-item">
                            <a class="nav-link active" href="{{route('recap.create')}}"><i class="fas fa-plus"></i></a>
                        </li>
                    </ul>
                    </div>
                </div>
                ...
                <div class="card-body">
                    <table class="table table-bordered table-striped" id="myTable">
                        ...
                        <tbody>
                            @foreach ($recap as $key=>$recaps)
                              <tr>
                                <td>{{$key+1}}</td>
                                <td>{{$recaps->periode}}</td>
                                <td>{{$recaps->year}}</td>
                                <td>
                                    <a href="{{route('recap.edit', $recaps->id)}}" class="btn btn-warning btn-sm">
                                      <i class="fas fa-edit"></i>
                                    </a>
                                    <form method="post" action="{{route('recap.destroy', $recaps->id)}}" class="d-inline">
                                      @csrf
                                      @method('DELETE')
                                    ...
                                    <a href="{{route('filerecap.index', $recaps->id)}}" class="btn btn-info btn-sm">
