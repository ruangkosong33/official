            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <ul class="nav nav-pills ml-auto">
                        <li class="nav-item">
                            <a class="nav-link active" href="{{route('auction.create')}}"><i class="fas fa-plus"></i></a>
                        </li>
                    </ul>
                    </div>
                </div>
                ...
                <div class="card-body">
                    <table class="table table-bordered table-striped" id="myTable">
                        ...
                        <tbody>
                            @foreach ($auction as $key=>$auctions)
                              <tr>
                                <td>{{$key+1}}</td>
                                <td>{{$auctions->title_auction}}</td>
                                <td><a href={{route('auction.download', $auctions->id)}}>{{$auctions->file_auction}}</a></td>
                                <td>
                                    <a href="{{route('auction.edit', $auctions->id)}}" class="btn btn-warning btn-sm">
                                      <i class="fas fa-edit"></i>
                                    </a>
                                    <form method="post" action="{{route('auction.destroy', $auctions->id)}}" class="d-inline">
                                      @csrf
                                      @method('DELETE')
