...
<div class="content-wrapper">
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Realisasi Fisik & Keuangan</h1>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="container-fluid">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Data Realisasi Fisik & Keuangan</h3>
                    <div class="card-tools">
                    <ul class="nav nav-pills ml-auto">
                        <li class="nav-item">
                            <a class="nav-link active" href="{{route('realisation.create')}}"><i class="fas fa-plus"></i></a>
                        </li>
                    </ul>
                    </div>
                </div>
                    ...
                    <table class="table table-bordered table-striped" id="myTable">
                        ...
                        <tbody>
                            @foreach ($realisation as $key=>$realisations)
                              <tr>
                                <td>{{$key+1}}</td>
                                <td>{{$realisations->title_realisation}}</td>
                                <td><a href="{{route('realisation.download', $realisations->id)}}">{{$realisations->file_realisation}}</a></td>
                                <td>
                                    <a href="{{route('realisation.edit', $realisations->id)}}" class="btn btn-warning btn-sm">
                                      <i class="fas fa-edit"></i>
                                    </a>
                                    <form method="post" action="{{route('realisation.destroy', $realisations->id)}}" class="d-inline">
                                      @csrf
                                      @method('DELETE')
