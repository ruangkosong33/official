    ...
    <section class="faq-search">
        <div class="container">
            <div class="section-title text-center">
                ...
                <h2 class="section-title__title">BBH KABUPATEN/KOTA</span></h2>
            </div>
        </div>
    </section>
    <!--Faq Search End-->
                        ...
                        @foreach ($bbas->filebba as $filebbas)
                            <tr>
                                ...
                                <td><a class="team-one__hover-sub-title"
                                        href="{{ route('landing.publicinfo.downloadFileBba', ['slug' => $filebbas->slug]) }}">{{ $filebbas->title_filebba }}</a>
                                </td>
                                <td>{{ $filebbas->description }}</td>
                                @php
                                    $date = \Carbon\Carbon::parse($filebbas->date);
                                @endphp
                                <td>{{ $date->format('d') . ' ' . $date->monthName . ', ' . $date->year }}</td>
                                <td>Rp. {{ number_format($filebbas->total, '0', ',', '.') }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
